...
public class BigDecimalMath {
    ...
    static public BigDecimal root(final int n, final BigDecimal x) {
         ...
         */
        Double num = x.ulp().doubleValue();
        if(num < Double.MIN_NORMAL){
            num = Double.MIN_NORMAL;
        }
        Double denom = 2.0d * (double) n * x.doubleValue();
        if(denom < Double.MIN_NORMAL){
            denom = Double.MIN_NORMAL;
        }
        ...
        for (; ; ) {
             ...
             */
            BigDecimal spownm1 = s.pow(n - 1);
            if (spownm1.doubleValue() < Double.MIN_NORMAL) {
                spownm1 = new BigDecimal(Double.MIN_NORMAL);
            }
            BigDecimal c = xhighpr.divide(spownm1, mc);
            c = s.subtract(c);
            MathContext locmc = new MathContext(c.precision());
            c = c.divide(nth, locmc);
            s = s.subtract(c);
            Double num2 = c.doubleValue();
            if(num2 < Double.MIN_NORMAL){
                num2 = Double.MIN_NORMAL;
            }
            Double denom2 = s.doubleValue();
            if(denom2 < Double.MIN_NORMAL){
                denom2 = Double.MIN_NORMAL;
            }
            Double diff = num2 / denom2;
            if(num2.equals(denom2)){
                diff = 1.0d;
            }
    ...
    static public BigDecimal log(BigDecimal x) {
        ...
        if (x.compareTo(BigDecimal.ZERO) < 0) {
            ...
            MathContext mc = null;
            try {
                mc = new MathContext(err2prec(res, diff));
