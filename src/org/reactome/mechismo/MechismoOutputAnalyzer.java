...
//        Total reactions having FIs in mechsimo: 1265
    ...
    public void checkSignificantReactions() throws IOException {
        ...
        // Note: The following file was generated by method checkReactionsInAnalysisResults().
        ...
        // Note: The following file was generated by the same method and saved manually
        // by copy/paste the output from Eclipse console.
    ...
    /**
     * This method is used to check reactions in the summary file generated by Franceso and used for
     * the manuscript. The original file name is: TCGA_mechismo_reactome_reactions.xlsx, downloaded
     * and then exported into tsv file locally with date stamped. Reactions in this file have been pre-
     * selected with FDR < 0.01.
     * @throws IOException
     */
    @Test
    public void checkReactionsInSummaryFile() throws IOException {
        // This file was exported from TCGA_mechismo_reactome_reactions_02052021.xlsx
        // It is the Reaction_ancertype_CGC sheet.
        String fileName = "results/TCGA_mechismo_reactome_reactions_02052021_Reaction_cancertype_CGC.txt";
        fileName = "results/tcga_mechismo_stat_cancer_wise_reactions_sig_CGC_021921.tsv";
        Set<String> reactionIds = Files.lines(Paths.get(fileName))
                                       .skip(1)
                                       .map(line -> line.split("\t"))
                                       .filter(tokens -> Double.parseDouble(tokens[11]) < 0.1d)
                                       .map(tokens -> tokens[1])
                                       .collect(Collectors.toSet());
        System.out.println("Total reaction ids: " + reactionIds.size());
//        if (true)
//            return;
        // Generate the list of significant reations for Cytoscape
//        fileName = "results/MechismoCancerTypeReactionList_020521.txt";
        String date = FileUtility.getDateStamp();
        fileName = "results/MechismoCancerTypeReactionList_" + date + ".txt";
        fu.setOutput(fileName);
        fu.printLine("ReactionID\tIsSignificant");
        for (String id : reactionIds)
            fu.printLine(id + "\ttrue");
        fu.close();
//        if (true)
//            return;
        // Generate the reaction sub-network
        ...
        ReactionMapGenerator mapGenerator = new ReactionMapGenerator();
        String outFileName = "results/MechismoCancerTypeReactionNetwork_" + date + ".txt";
        PrintStream ps = new PrintStream(outFileName);
        mapGenerator.generateSubNetwork(reactionIds, ps);
        ps.close();
        outFileName = "results/MechismoCancerTypeReactionConnectedNetwork_" + date + ".txt";
        ps = new PrintStream(outFileName);
        mapGenerator.generateSubNetworkForAll(reactionIds, new HashMap<>(), ps);
        ps.close();
    }
