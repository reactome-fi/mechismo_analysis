...
input_csvs <- list.files("/media/burkhart/Media/Software/Ogmios/results/knownDriverData/",pattern = "firehose_.*_mutationDensity.csv",full.names = TRUE)
...
processed_ppi_sample_df <- sample_df %>%
  dplyr::select(CancerType,Gene.Symbol,Mutation.Density) %>%
  dplyr::mutate(N.Log10.Mutation.Density = -log10(Mutation.Density + 0.00001)) %>%
  #dplyr::filter(FDR < 0.05) %>%
  dplyr::select(CancerType,Gene.Symbol,N.Log10.Mutation.Density)

ppi_dendro <- processed_ppi_sample_df %>%
  reshape(idvar = "CancerType", timevar = "Gene.Symbol", direction = "wide")

names(ppi_dendro) <- gsub("N.Log10.Mutation.Density.(.*)","\\1",names(ppi_dendro))
...
pheatmap::pheatmap(m,
                   ...
                   main = "Known Driver Hotspot -log10(Mutation Density) for Sliding Window of 300 AA",
