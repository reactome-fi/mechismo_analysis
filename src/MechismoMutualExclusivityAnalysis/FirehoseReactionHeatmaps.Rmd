...
# load ACC
```{r}
#input_csv_path = "/home/burkhart/Software/Ogmios/results/heatmapData/firehose_UVM_reactions_with_mutated_interfaces.csv"
sample_df <- read.csv(input_csv_path,header = TRUE, sep="~")
```
# filter low 
```{r}
processed_ppi_sample_df <- sample_df %>%
  dplyr::select(Sample,Reaction.ID,P.Value) %>%
...
processed_ppi_sample_df %>%
  ggplot2::ggplot(aes(x=as.character(Sample),y=as.character(Reaction.ID),fill=P.Value)) +
  geom_tile() +
  ylim(rev(sort(unique(as.character(processed_ppi_sample_df$Reaction.ID))))) +
  xlim(sort(unique(as.character(processed_ppi_sample_df$Sample)))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank()) +
  labs(title="Sample/Reaction p-value",
       x="Sample",
       y="Reaction.ID") +
...
ppi_dendro <- processed_ppi_sample_df %>%
  reshape(idvar = "Sample", timevar = "Reaction.ID", direction = "wide")
